...
const createUsername = (fName, lName) => {
    const fullNameLength = `${fName}${lName}`.length
    const fNameLetters = fName.slice(0, 3)
    const lNameLetters = lName.slice(0, 3)
    const userName = `${fNameLetters}${lNameLetters}${fullNameLength}`
